...
function main(argv) {
  ...
  const useViteNodeEntrypoint = process.platform === "win32" && command === "vite";
  const executable = useViteNodeEntrypoint
    ? process.execPath
    : command;
  const childArgs = useViteNodeEntrypoint
    ? [fileURLToPath(new URL("../node_modules/vite/bin/vite.js", import.meta.url)), ...args]
    : args;
